...
//парсим (переводим из строки в объект)
let superheroesArr = JSON.parse(json);

//добавляем данные из массива на страницу
document.addEventListener("DOMContentLoaded", function (event) {
  console.log(superheroesArr);
  let cardSuperheroesContent = "";
  for (let item of superheroesArr) {
    cardSuperheroesContent += `
    <div id="cardSuperheroes__container">
      <h1 id="cardSuperheroes__name">${item.name}</h1>
      <div id="cardSuperheroes__universe"><span class="cardSuperheroes__text">Вселенная:</span> ${item.universe}</div>
      <div id="cardSuperheroes__alterego"><span class="cardSuperheroes__text">Альтер эго:</span> ${item.alterego}</div>
      <div id="cardSuperheroes__occupation"><span class="cardSuperheroes__text">Род деятельности:</span> ${item.occupation}</div>
      <div id="cardSuperheroes__friends"><span class="cardSuperheroes__text">Друзья:</span> ${item.friends}</div>
      <div id="cardSuperheroes__superpowers"><span class="cardSuperheroes__text">Суперсилы:</span> ${item.superpowers}</div>
      <img id="cardSuperheroes__img" src="${item.url}">
      <div class="cardSuperheroes__rating-area">
        <input class="cardSuperheroes__star" type="radio" id="star-5${item.name}" name="rating-${item.name}" value="5">
        <label for="star-5${item.name}" title="Оценка «5»"></label>
        <input class="cardSuperheroes__star" type="radio" id="star-4${item.name}" name="rating-${item.name}" value="4">
        <label for="star-4${item.name}" title="Оценка «4»"></label>
        <input class="cardSuperheroes__star" type="radio" id="star-3${item.name}" name="rating-${item.name}" value="3">
        <label for="star-3${item.name}" title="Оценка «3»"></label>
        <input class="cardSuperheroes__star" type="radio" id="star-2${item.name}" name="rating-${item.name}" value="2">
        <label for="star-2${item.name}" title="Оценка «2»"></label>
        <input class="cardSuperheroes__star" type="radio" id="star-1${item.name}" name="rating-${item.name}" value="1">
        <label for="star-1${item.name}" title="Оценка «1»"></label>
      </div>
    </div>
    `;
  }
  document.getElementById("cardSuperheroes").innerHTML = cardSuperheroesContent;

  let inputStars = document.querySelectorAll(".cardSuperheroes__star");

  // Обработчик события изменения рейтинга
  inputStars.forEach(function (star) {
    star.addEventListener("change", function (event) {
      let ratingValue = event.target.value; //значение рейтинга
      let superheroName = event.target.id.slice(6); //имя супергероя
      // Сохраняем рейтинг в локальном хранилище
      localStorage.setItem(superheroName, ratingValue);
    });
  });
});
